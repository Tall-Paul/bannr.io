...
editorAPI.returnTemplate = function(template_id,callback_func){
  var template = editorAPI.loadedTemplates[template_id];
  callback_func(template);
}
...
editorAPI.returnCampaign = function(campaign_id,callback_func){
  console.log(editorAPI.loadedCampaigns);
  var campaign = editorAPI.loadedCampaigns[campaign_id];
  callback_func(campaign);
}

editorAPI.returnTemplateWithData = function(template_name,data,callback_func){
  $.each(editorAPI.loadedTemplates,function(el,ind,arr){
    if (template_name == ind.label){
        ind.data = data;
        callback_func(ind);
    }
...
editorAPI.loadTemplateWithData = function(site_id,template_name,data,callback_func){
    if (editorAPI.loadedTemplates === null){
      $.getJSON('/api/sites/'+site_id+'/templates',function(templates){
        editorAPI.loadedTemplates = templates;
        editorAPI.returnTemplateWithData(template_name,data,callback_func);
      });
    } else {
      editorAPI.returnTemplateWithData(template_name,data,callback_func);
    }
