...
class ProxyController extends Controller
{
   ...
   protected function proxy_curl($url){
    ...
    public function index($site_id)
    {
        ...
        ",$response);
        $replacement = "href='https://$site_id.proxy.bannr.io";
        $response = str_replace('href="'.$proxy_url,$replacement,$response);
        $response = str_replace("href='".$proxy_url,$replacement,$response);
        $non_ssl = str_replace('https','http',$proxy_url);
        $response = str_replace('href="'.$non_ssl,$replacement,$response);
        $response = str_replace("href='".$non_ssl,$replacement,$response);
